...
locals {
  ...
  github_webhooks_token = length(
    join("", data.aws_ssm_parameter.github_webhooks_token.*.value),
  ) > 0 ? join("", data.aws_ssm_parameter.github_webhooks_token.*.value) : var.github_webhooks_token
  github_webhooks_token_ssm_name = length(var.github_webhooks_token_ssm_name) > 0 ? var.github_webhooks_token_ssm_name : format(
    var.chamber_format,
    var.chamber_service,
...
locals {
  authentication_cognito_user_pool_arn = length(
    join(
      "",
      data.aws_ssm_parameter.atlantis_cognito_user_pool_arn.*.value,
    ),
    ) > 0 ? join(
    "",
    data.aws_ssm_parameter.atlantis_cognito_user_pool_arn.*.value,
  ) : var.authentication_cognito_user_pool_arn
  authentication_cognito_user_pool_arn_ssm_name = length(var.authentication_cognito_user_pool_arn_ssm_name) > 0 ? var.authentication_cognito_user_pool_arn_ssm_name : format(
    var.chamber_format,
    var.chamber_service,
    "atlantis_cognito_user_pool_arn",
  )

  authentication_cognito_user_pool_client_id = length(
    join(
      "",
      data.aws_ssm_parameter.atlantis_cognito_user_pool_client_id.*.value,
    ),
    ) > 0 ? join(
    "",
    data.aws_ssm_parameter.atlantis_cognito_user_pool_client_id.*.value,
  ) : var.authentication_cognito_user_pool_client_id
  authentication_cognito_user_pool_client_id_ssm_name = length(var.authentication_cognito_user_pool_client_id_ssm_name) > 0 ? var.authentication_cognito_user_pool_client_id_ssm_name : format(
    var.chamber_format,
    var.chamber_service,
    "atlantis_cognito_user_pool_client_id",
  )

  authentication_cognito_user_pool_domain = length(
    join(
      "",
      data.aws_ssm_parameter.atlantis_cognito_user_pool_domain.*.value,
    ),
    ) > 0 ? join(
    "",
    data.aws_ssm_parameter.atlantis_cognito_user_pool_domain.*.value,
  ) : var.authentication_cognito_user_pool_domain
  authentication_cognito_user_pool_domain_ssm_name = length(var.authentication_cognito_user_pool_domain_ssm_name) > 0 ? var.authentication_cognito_user_pool_domain_ssm_name : format(
    var.chamber_format,
    var.chamber_service,
    "atlantis_cognito_user_pool_domain",
  )

  authentication_oidc_client_id = length(
    join("", data.aws_ssm_parameter.atlantis_oidc_client_id.*.value),
  ) > 0 ? join("", data.aws_ssm_parameter.atlantis_oidc_client_id.*.value) : var.authentication_oidc_client_id
  authentication_oidc_client_id_ssm_name = length(var.authentication_oidc_client_id_ssm_name) > 0 ? var.authentication_oidc_client_id_ssm_name : format(
    var.chamber_format,
    var.chamber_service,
    "atlantis_oidc_client_id",
  )

  authentication_oidc_client_secret = length(
    join(
      "",
      data.aws_ssm_parameter.atlantis_oidc_client_secret.*.value,
    ),
    ) > 0 ? join(
    "",
    data.aws_ssm_parameter.atlantis_oidc_client_secret.*.value,
  ) : var.authentication_oidc_client_secret
  authentication_oidc_client_secret_ssm_name = length(var.authentication_oidc_client_secret_ssm_name) > 0 ? var.authentication_oidc_client_secret_ssm_name : format(
    var.chamber_format,
    var.chamber_service,
